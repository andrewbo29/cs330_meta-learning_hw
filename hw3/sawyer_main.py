    ...
    for t in range(STEPS_PER_EPISODE) :
        ...
        if HER == 'None':
            ...
            for _ in range(num_relabeled):
                her_goal = episode_experience[-1][3][:NUM_DIM]
                her_reward = -1 * euclidean(inputs[:NUM_DIM], her_goal)
                inputs = np.concatenate((inputs[:NUM_DIM], her_goal))
                inputs_ = np.concatenate((inputs_[:NUM_DIM], her_goal))
            ...
            # pass
            for _ in range(num_relabeled):
                future_state = np.random.choice(range(t, STEPS_PER_EPISODE), 1)[0]
                her_goal = episode_experience[future_state][3][:NUM_DIM]
                her_reward = -1 * euclidean(inputs[:NUM_DIM], her_goal)
                inputs = np.concatenate((inputs[:NUM_DIM], her_goal))
                inputs_ = np.concatenate((inputs_[:NUM_DIM], her_goal))
                replay_buffer.add(inputs, a, her_reward, inputs_)
            ...
            # pass
            for _ in range(num_relabeled):
                random_state = np.random.choice(range(STEPS_PER_EPISODE ), 1)[0]
                her_goal = episode_experience[random_state][3][:NUM_DIM]
                her_reward = -1 * euclidean(inputs[:NUM_DIM], her_goal)
                inputs = np.concatenate((inputs[:NUM_DIM], her_goal))
                inputs_ = np.concatenate((inputs_[:NUM_DIM], her_goal))
                replay_buffer.add(inputs, a, her_reward, inputs_)
