    ...
    for t in range(num_bits) :
        ...
        if HER == 'None':
            ...
            for _ in range(num_relabeled):
                her_goal = episode_experience[-1][3][:num_bits]
                her_reward = 0
                inputs = np.concatenate((inputs[:num_bits], her_goal))
                inputs_ = np.concatenate((inputs_[:num_bits], her_goal))
                replay_buffer.add(inputs, a, her_reward, inputs_)
            ...
            # pass
            for _ in range(num_relabeled):
                future_state = np.random.choice(range(t, num_bits), 1)[0]
                her_goal = episode_experience[future_state][3][:num_bits]
                her_reward = 0
                inputs = np.concatenate((inputs[:num_bits], her_goal))
                inputs_ = np.concatenate((inputs_[:num_bits], her_goal))
                replay_buffer.add(inputs, a, her_reward, inputs_)
            ...
            # pass
            for _ in range(num_relabeled):
                random_state = np.random.choice(range(num_bits), 1)[0]
                her_goal = episode_experience[random_state][3][:num_bits]
                her_reward = 0
                inputs = np.concatenate((inputs[:num_bits], her_goal))
                inputs_ = np.concatenate((inputs_[:num_bits], her_goal))
                replay_buffer.add(inputs, a, her_reward, inputs_)
...
def flip_bits(HER = "None"):
    ...
    for i in range(num_epochs):
        ...
        for k in range(FLAGS.opt_steps):
            ...
            # calculate loss
            # print(state.shape, np.reshape(action,[-1]).shape, target_reward.shape)
