...
if __name__ == '__main__':
	...
	for ep in range(n_epochs):
		for epi in range(n_episodes):
			...
			support = support_vec_sort.reshape((n_way, k_shot, im_width, im_height, channels))
	...
	for epi in range(n_meta_test_episodes):
		...
		images_batch, labels_batch = data_generator.sample_batch('meta_test', 1)

		support = images_batch[0, :, :k_meta_test_shot, :].reshape((n_meta_test_way, k_meta_test_shot, im_width,
																	im_height, channels))
		query = images_batch[0, :, k_meta_test_shot:, :].reshape((n_meta_test_way, n_meta_test_query, im_width,
																  im_height, channels))

		labels_s = labels_batch[0, :, :k_meta_test_shot, :].reshape((n_meta_test_way, k_meta_test_shot, n_meta_test_way))
		labels = labels_batch[0, :, k_meta_test_shot:, :].reshape((n_meta_test_way, n_meta_test_query, n_meta_test_way))

		support_vec = support.reshape((n_meta_test_way * k_meta_test_shot, im_width * im_height * channels))
		labels_s_vec = labels_s.reshape((n_meta_test_way * k_meta_test_shot, n_meta_test_way))

		l_s_argmax = labels_s_vec.argmax(1)
		sort_ind = np.argsort(l_s_argmax)
		support_vec_sort = support_vec[sort_ind]

		support = support_vec_sort.reshape((n_meta_test_way, k_meta_test_shot, im_width, im_height, channels))
