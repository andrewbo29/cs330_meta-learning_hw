...
from utils import xent, conv_block, accuracy
...
class MAML:
	...
	def construct_model(self, prefix='maml'):
		...
		with tf.variable_scope('model', reuse=None) as training_scope:
			...
			# if 'inner_lr' in dir(self):
			# 	training_scope.reuse_variables()
			# 	inner_lr = self.inner_lr
			# else:
			# 	self.inner_lr = inner_lr = self.construct_inner_lr(weights)
			...
			def task_inner_loop(inp, reuse=True):
				...
				inputa = tf.reshape(inputa, [-1, 784])
				labela = tf.reshape(labela, [-1, self.dim_output])

				inputb = tf.reshape(inputb, [-1, 784])
				labelb = tf.reshape(labelb, [-1, self.dim_output])

				task_outputa = self.forward_conv(inputa, weights, reuse)
				task_lossa = self.loss_func(task_outputa, labela)
				task_accuracya = accuracy(task_outputa, labela)

				for i in range(num_inner_updates):
					out_a = self.forward_conv(inputa, weights, reuse=True)
					loss_a = self.loss_func(out_a, labela)
					for key in weights:
						weights[key] = weights[key] - tf.math.scalar_mul(self.inner_update_lr,
																		 tf.gradients(loss_a, weights[key])[0])

						# lr = inner_lr['{}_lr'.format(key)]
						# weights[key] = weights[key] - tf.math.multiply(lr, tf.gradients(loss_a, weights[key])[0])

					task_outputb = self.forward_conv(inputb, weights, reuse=True)
					task_outputbs.append(task_outputb)
					task_lossesb.append(self.loss_func(task_outputb, labelb))
					task_accuraciesb.append(accuracy(task_outputb, labelb))
	...
	def construct_inner_lr(self, weights):
		uniform_initializer = tf.random_uniform_initializer(minval=0.01, maxval=0.5)

		inner_lr = {}
		for key in weights:
			name = '{}_lr'.format(key)
			inner_lr[name] = tf.get_variable(name, [1], initializer=uniform_initializer, dtype=tf.float32)
