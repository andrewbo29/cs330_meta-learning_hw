...
def meta_train(model, saver, sess, exp_string, data_generator, resume_itr=0):
	...
	for itr in range(resume_itr, FLAGS.meta_train_iterations):
		...
		images_batch, labels_batch = data_generator.sample_batch('meta_train', FLAGS.meta_batch_size)

		split_num = int(data_generator.num_samples_per_class / 2)

		inputa = images_batch[:, :, :split_num, :]
		labela = labels_batch[:, :, :split_num, :]

		inputb = images_batch[:, :, split_num:, :]
		labelb = labels_batch[:, :, split_num:, :]
		...
		if (itr!=0) and itr % TEST_PRINT_INTERVAL == 0:
			...
			images_batch, labels_batch = data_generator.sample_batch('meta_val', FLAGS.meta_batch_size)
...
def meta_test(model, saver, sess, exp_string, data_generator, meta_test_num_inner_updates=None):
	...
	for _ in range(NUM_META_TEST_POINTS):
		...
		images_batch, labels_batch = data_generator.sample_batch('meta_test', FLAGS.meta_batch_size)
